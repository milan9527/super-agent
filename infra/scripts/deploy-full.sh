...
if [ "$SKIP_AGENTCORE" = false ]; then
  ...
  if [ -n "$LITELLM_URL" ]; then
    ...
    ENV_VARS="{\"CLAUDE_CODE_USE_BEDROCK\":\"0\",\"ANTHROPIC_API_KEY\":\"${LITELLM_KEY}\",\"ANTHROPIC_BASE_URL\":\"${ANTHROPIC_URL}\",\"ANTHROPIC_MODEL\":\"${EFFECTIVE_MODEL}\",\"ANTHROPIC_CUSTOM_MODEL_OPTION\":\"${EFFECTIVE_MODEL}\",\"USE_API_CONTEXT_MANAGEMENT\":\"0\",\"CLAUDE_CODE_THINKING_TYPE\":\"adaptive\",\"AWS_REGION\":\"$REGION\",\"WORKSPACE_S3_REGION\":\"$REGION\""
  else
